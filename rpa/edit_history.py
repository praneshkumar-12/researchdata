...
def record_update(uniqueid, name, updates, db_object):
    ...
    old_record_dict = {field.name: getattr(old_record_object, field.name) for field in old_record_object._meta.fields}
    modified = compare_dicts(old_record_dict,updates)

    for key, (old_value,new_value) in modified.items():
        new_record = Edithistory(uniqueid=old_record_object, edit_timestamp=timestamp, edited_by=name,
...
def compare_dicts(old_dict,new_dict):
    ...
    modified_dict = {o : (old_dict[o], new_dict[o]) for o in shared_keys if old_dict[o] != new_dict[o]}
